...
async function callPhaseCPerplexitySafe(systemPrompt, userPrompt, options) {
  ...
  const useChunked = opts.chunked !== false;
  if (!useChunked) {
...
const SYSTEM_PROMPT = [
  ...
  'Respond ONLY with valid JSON (no markdown fences, no commentary). Your entire reply MUST be one parseable JSON object using exactly these keys:',
