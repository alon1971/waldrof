 ...
 * GEMINI_API_KEY: pedagogical side-chat (chat_followup) + enrichment_links (phase_c inspiration) ONLY.
 *
...
const WEB_SEARCH_PRIORITY_INSTRUCTION =
  ...
  'FOR INSPIRATION / PHASE C: run MANDATORY site-restricted Google searches FIRST on the Waldorf domain whitelist:\n' +
  '  site:waldorf.org.il, site:adamolam.co.il, site:shakedwaldorf.org.il, site:harduf-waldorf.org.il, ' +
  '  plus «הרדוף חינוך וולדורף», «סמינר שילוב», «סמינר דוד ילין וולדורף» combined with the block topic.\n' +
  'Then broaden to AWSNA, Waldorf World, Goetheanum, waldorflibrary.org — collect HTTPS deep links ONLY when verbatim from live citations.\n' +
  'NEVER invent static URLs or search-index paths — safe Google site: fallbacks are injected server-side when needed.\n' +
...
const ENRICHMENT_LINKS_MAX = 5;

const ENRICHMENT_LINKS_GEMINI_SYSTEM_INSTRUCTION =
  '\n=== ENRICHMENT LINKS — GEMINI-ONLY 5+5 STRATEGY (MANDATORY) ===\n' +
  'This pipeline is STRICTLY DECOUPLED from live web search — identical isolation to pedagogical chat.\n' +
  'Do NOT perform, simulate, cite, or merge Perplexity, Sonar, live web search, search payloads, or citation blocks.\n' +
  'Populate enrichment_links using ONLY your internal global knowledge of Waldorf/Steiner pedagogy, ' +
  'well-known trusted repositories, and structurally stable URL patterns you are confident about.\n' +
  '• "pinterest_links" (up to 5): DISTINCT, high-quality Pinterest SEARCH URLs only — full https://www.pinterest.com/search/pins/?q=…\n' +
  '  Craft grade-locked English queries: Waldorf Class N {englishTopic} — max 2–4 keywords per query; vary angle (main lesson book, chalkboard, topic focus).\n' +
  '  Each item: { "title": "Hebrew label", "url": "https://www.pinterest.com/search/pins/?q=…", "query": "Waldorf Class 8 revolutions" }\n' +
  '• "article_links" (up to 5): DISTINCT, stable HTTPS URLs from VERIFIED trusted Waldorf repositories ONLY.\n' +
  '  Priority: waldorflibrary.org, zomer.org.il, adamolam.co.il, elyashev.co.il, waldorf-forum.org.il, anatta.co.il, ' +
  'AWSNA (waldorfeducation.org), Goetheanum, rsarchive.org — deep article/PDF pages or stable site-restricted search URLs ' +
  'you know from Waldorf pedagogy knowledge (never invented paths).\n' +
  '  Each item: { "title": "Hebrew page title", "url": "https://…", "source": "domain/org name", "label": "מאמר פדגוגי|כתב עת פדגוגי|…", "snippet": "Hebrew relevance to currentGrade + topic" }\n' +
  'STRICT QUALITY GUARDRAIL (NON-NEGOTIABLE):\n' +
  'The AI should optimize for quality and relevance over quantity. If 5 verified, high-value distinct links cannot be found or formulated for the given topic, ' +
  'the AI must only return the high-quality ones it is certain about (e.g., 3 or 4), rather than providing generic or filler links just to reach the number 5.\n' +
  'FORBIDDEN: generic education blogs, direct Pinterest board/pin URLs (search URLs only), fabricated Israeli school paths, duplicate URLs, wrong-grade Pinterest queries, filler links.\n' +
  '=== END ENRICHMENT LINKS — GEMINI-ONLY ===\n';
...
const PERPLEXITY_PHASE_C_INSPIRATION_NO_LINKS_INSTRUCTION =
  ...
  'enrichment_links and pedagogicalResources are generated by a SEPARATE Gemini pipeline — do NOT include them in this response.\n' +
...
const WALDORF_PEDAGOGICAL_DOMAIN_HINTS = [
  ...
  { pattern: /shakedwaldorf\.org\.il|shakedtivon\.edupage\.org/i, source: 'בית ספר שקד', label: 'מערך שיעור מאתר בית ספר' },
  { pattern: /harduf-waldorf\.org\.il/i, source: 'בית ספר ולדורף הרדוף', label: 'מערך שיעור מאתר בית ספר' },
  { pattern: /google\.com\/search\?.*site/i, source: 'חיפוש מוגבל לאתר', label: 'מקור וולדורף רשמי' },
  ...
  { pattern: /adam-?olam|אדם.?עולם/i, source: 'כתב עת אדם עולם', label: 'כתב עת פדגוגי' },
...
function normalizePedagogicalResourceItem(item, body) {
  ...
  if (waldorfWebSeed.isSafeGoogleSiteSearchUrl(url) && !snippet) {
    snippet = gradeLabel
      ? ('חיפוש Google מוגבל לאתר — נושא «' + topic + '» ב' + gradeLabel)
      : ('חיפוש Google מוגבל לאתר — נושא «' + topic + '»');
  }
  if (topic && snippet && !waldorfWebSeed.isSafeGoogleSiteSearchUrl(url) &&
      !waldorfWebSeed.looksLikeVerifiedDeepLink(url)) {
...
function buildGeminiEnrichmentLinksSystemPrompt() {
  return (
    'You are an expert Waldorf / Steiner-Waldorf pedagogy curator for visual and bibliographic enrichment links.\n' +
    pedagogicalScope.PEDAGOGICAL_SCOPE_GUARDRAIL_INSTRUCTION +
    STEINER_ANTHROPOSOPHIC_FIDELITY_INSTRUCTION +
    ENRICHMENT_LINKS_GEMINI_SYSTEM_INSTRUCTION +
...
function buildGeminiEnrichmentLinksUserPrompt(body) {
  const topic = String(body.topic || '').trim().replace(/"/g, '');
  return (
    buildGradeLockBlock(body) +
    buildLanguageBlock(body) +
    '\n=== ENRICHMENT LINKS REQUEST (GEMINI KNOWLEDGE BASE ONLY) ===\n' +
    'Generate enrichment_links for the active Waldorf main-lesson block.\n' +
    'Do NOT use Perplexity, Sonar, live web search, or any external search payload — internal Waldorf knowledge only.\n' +
    ...
    '{\n' +
    '  "enrichment_links": {\n' +
    '    "pinterest_links": [{ "title": "Hebrew label", "url": "https://www.pinterest.com/search/pins/?q=…", "query": "Waldorf Class N topic" }],\n' +
    '    "article_links": [{ "title": "Hebrew page title", "url": "https://…", "source": "waldorflibrary.org", "label": "מאמר פדגוגי", "snippet": "Hebrew relevance" }]\n' +
    '  }\n' +
    '}\n' +
    '=== END ENRICHMENT LINKS REQUEST ===\n'
  );
}

function getGeminiEnrichmentLinksResponseSchema() {
  return {
    type: 'object',
    properties: {
      enrichment_links: {
        type: 'object',
        properties: getEnrichmentLinksSchemaProperties(),
...
async function fetchGeminiEnrichmentLinks(body) {
  const systemPrompt = buildGeminiEnrichmentLinksSystemPrompt();
  const userPrompt = buildGeminiEnrichmentLinksUserPrompt(body);
  let lastErr = null;

  for (let attempt = 1; attempt <= MODEL_PARSE_MAX_ATTEMPTS; attempt++) {
    try {
      if (attempt > 1) {
        console.warn('[gemini] enrichment_links retry (attempt', attempt + '/' + MODEL_PARSE_MAX_ATTEMPTS + ')');
      }
      console.log('[gemini] enrichment_links pipeline for phase_c inspiration');
      const raw = await chatApi.callGeminiV1(systemPrompt, userPrompt, {
        jsonMode: true,
        responseSchema: getGeminiEnrichmentLinksResponseSchema(),
      ...
      return normalizeEnrichmentLinks(links, body, { geminiOnly: true });
    } catch (err) {
      lastErr = err;
      if (attempt >= MODEL_PARSE_MAX_ATTEMPTS || isNonRetriableApiClientError(err)) break;
    }
  }

  throw lastErr || new Error('Gemini enrichment_links generation failed');
}
...
async function attachGeminiEnrichmentLinks(data, body) {
  ...
  try {
    data.enrichment_links = await fetchGeminiEnrichmentLinks(body);
    ...
    console.warn('[gemini] enrichment_links pipeline failed:', msg);
...
function buildPerplexitySearchSystemPrompt() {
  return (
    ...
    'Israeli Waldorf schools (שקד, חרדוף, waldorf.org.il), journals such as Adam Olam («אדם עולם»), ' +
    'and verified anthroposophic pedagogical sources. ' +
...
async function executeGenerate(body, apiKey, requestContext) {
  ...
  // Main generation: grade/topic/phase_c → Perplexity Sonar + synthesis; phase_c inspiration enrichment_links → Gemini only; chat → Gemini.
    ...
    (body.phase === 'phase_c'
      ? ' PHASE C — INDEPENDENT TAB («' + body.cTab + '»): Do NOT duplicate or paraphrase Phase B theory essence. Generate unique, deep tab-specific content from Perplexity research only.' +
        (resolvePhaseCTab(body) === 'inspiration'
          ? ' enrichment_links are GEMINI-ONLY — Perplexity must NOT output link arrays or citation URLs.'
          : '')
      : '') +
    (isOnDemandExpansion
      ...
      : isDecoupledGen
      ? ' DECOUPLED MAIN GENERATION: Perplexity Sonar research → Perplexity text synthesis. enrichment_links (phase_c inspiration) → separate Gemini pipeline. No Drive/community RAG injection.'
      : isChatFollowup
