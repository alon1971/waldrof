...
function buildCommunitySummarySystemPrompt() {
  return [
    ...
    'תפקידך להפיק סיכום עמוק, מקיף, מפורט ועשיר פדגוגית מתוך חומרי המאגר הקהילתי ב-Google Drive בלבד.',
    'אסור להסתמך על ידע חיצוני או על חיפוש ברשת — רק על הטקסטים/המסמכים שסופקו.',
    'כתוב בעברית פדגוגית עשירה, רהוטה ומדויקת — לא רשימות דלות, לא תקציר שטחי של 5 שורות, ולא דילוג על חלקים מרכזיים מהמקורות.',
    'דרישת אורך: כשיש תוכן במקורות, הסיכום חייב להיות מפורט ומקיף — מינימום 400–600 מילים (רצוי יותר אם החומר מאפשר). אסור להסתפק בסקירה קצרה.',
    'הסיכום חייב להיות thorough ומלא: רקע רעיוני, רציונל פדגוגי וולדורפי, מושגי מפתח, יישומים מעשיים בכיתה, קצב/מבנה שיעור או תקופה כשמופיע במקורות, והמלצות למורה.',
    'חובה להשלים את כל הסעיפים עד הסוף — אל תעצור באמצע משפט, אל תקצר בגלל אורך, ואל תדלג על סעיפים חשובים.',
    'מבנה Markdown חובה בתוך שדה summary: כותרת ראשית (#), כותרות משנה (## / ###), נקודות־תבליט, והדגשת מושגי מפתח ב־**מודגש**.',
    'בהדגשות Markdown שמור על זוגות תקינים של **…** באותה יחידת טקסט — אל תשבור ** באמצע שורה ללא סגירה.',
    'בסוף הסיכום הוסף סעיף «### מראי מקום» והשלם אותו במלואו.',
    ...
    'ודא שה-JSON שלם וסגור (סוגריים ומירכאות) ושדה summary מכיל את הסיכום המלא מההתחלה ועד מראי המקום.',
  ].join(' ');
}
...
function buildCommunitySummaryTextPreamble(query, options, fileBundles, mediaMeta) {
  ...
  return [
    ...
    '',
    'הפק סיכום פדגוגי עמוק, מקיף, מפורט ומאוחד של החומרים הבאים — לא תקציר קצר של כמה שורות.',
    'כשיש חומר במקורות: כתוב סיכום מפורט באורך מינימום 400–600 מילים (או יותר לפי עומק החומר).',
    ...
    '',
    'פורמט פלט חובה: החזר רק JSON תקין ושלם במבנה {"summary":"<טקסט הסיכום בעברית עם Markdown>"}.',
