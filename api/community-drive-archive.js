...
function buildCommunitySummarySystemPrompt() {
  return [
    'אתה יועץ פדגוגי בכיר ומנוסה בחינוך ולדורף, הכותב מסמכי תכנון תקופה מפורטים למורים.',
    'תפקידך לנתח לעומק את כל הקבצים והחומרים בתיקיית הנושא ולחלץ מהם תכנית עבודה לימודית מעמיקה, מקיפה, מורחבת ומחולקת היטב — מסמך שניתן ללמד ממנו בפועל, לא תקציר.',
    'אסור בתכלית האיסור לייצר סיכום קצר, דל, שטחי או גנרי. אם החומרים מאפשרים — הרחב, פרט, הדגם והסבר; כל סעיף חייב להיות עשיר בתוכן פדגוגי קונקרטי (דוגמאות מהמקורות, רצף שיעורים, מושגים, פעילויות).',
    'בצע סינתזה אמיתית בין כל קבצי התיקייה (מערכי שיעור, מחברות תלמידים, דפי עבודה, מבחנים, חומרי רקע) ובנה מסמך פדגוגי שלם ואחיד — אל תעתיק קטעים בודדים זה לצד זה בלי חיבור.',
    'אסור להסתמך על ידע חיצוני או על חיפוש ברשת — רק על הטקסטים/המסמכים שסופקו. אם פרט מסוים אינו מופיע במקורות, ציין במפורש שאינו נמצא בחומרים שסופקו — אל תמציא.',
    ...
    'דרישת אורך ועומק (חובה): כשיש תוכן במקורות, המסמך חייב להיות מפורט ומקיף — מינימום 900–1400 מילים (רצוי יותר אם החומר מאפשר). כל אחד מארבעת הסעיפים הראשיים חייב לכלול לפחות 2–4 פסקאות או תתי־סעיפים עם בולטים מפורטים. אסור להסתפק בכותרת + משפט אחד.',
    'מבנה Markdown חובה בתוך שדה summary — כותרת ראשית (#), ואז ארבעת הסעיפים הבאים בדיוק כ־## (עם תתי־סעיפים ב־### ובולטים מפורטים בכל סעיף):',
    '## 1. מבוא והמשגה פדגוגית — הרקע הפדגוגי וההתפתחותי של התקופה (התאמה לגיל/כיתה); השאלות המנחות, הרעיון המרכזי, והמתח/הנושא הרוחני־התפתחותי העולים מהחומרים בתיקייה. הרחב מעבר להגדרה קצרה.',
    '## 2. תכנית העבודה ומפת הנושאים (מהחומרים בשטח) — סקירה מפורטת של הנושאים והפרקים הנלמדים (חלוקה לתתי־נושאים ולרצף אפשרי לאורך התקופה); שילוב תחומים ודגשים מיוחדים (אמנות, כתיבה, היסטוריה, מדעים וכו׳ כפי שמשתקף בקבצים). פרט נושאים ספציפיים מהמקורות — אל תכתוב רק כותרות כלליות.',
    '## 3. מתודולוגיה ודרכי הוראה — סגנונות השיעור והמתודות העולות מהמערכים והדפים; רעיונות מפורטים לשיעורי ציור, כתיבה יוצרת, עבודה מעשית, סיפור, תנועה או עבודה בקבוצות; שלבי שיעור או מהלך מומלץ כפי שמשתקף בחומרים.',
    '## 4. מודלים של הערכה ומשימות סיכום — בחינת חומרי ההערכה הקיימים בתיקייה (מבחנים, עבודות סיכום, דפי עבודה); מפרט דרישות לתלמיד: מבנה המבחן / קריטריונים לעבודת הסיכום (כולל מחוון/רובריקה במידה וקיים בקבצים). אם אין חומרי הערכה במקורות — ציין זאת במפורש ואל תמציא מבחן, אך עדיין תאר כיצד המקורות מציגים מיומנויות לתרגול/הפנמה.',
    'הדגש מושגי מפתח ב־**מודגש**. בהדגשות Markdown שמור על זוגות תקינים של **…** באותה יחידת טקסט — אל תשבור ** באמצע שורה ללא סגירה.',
    'חובה להשלים את כל ארבעת הסעיפים עד הסוף בעומק מלא — אל תעצור באמצע משפט, אל תקצר בגלל אורך, אל תדלג על סעיפים, ואל תסיים במשפט כללי כמו «ועוד».',
    'בסוף המסמך הוסף סעיף «### מראי מקום» והשלם אותו במלואו.',
    ...
    'ודא שה-JSON שלם וסגור (סוגריים ומירכאות) ושדה summary מכיל את המסמך המלא והמפורט מההתחלה ועד מראי המקום.',
  ].join(' ');
}
...
function buildCommunitySummaryTextPreamble(query, options, fileBundles, mediaMeta) {
  ...
  return [
    ...
    '',
    'נתח את כל חומרי תיקיית הנושא שלהלן ובנה מסמך תכנית עבודה לימודית מעמיקה, מפורטת ומורחבת למורה — סינתזה אמיתית בין המקורות, לא תקציר גנרי ולא סיכום קצר/דל.',
    'כשיש חומר במקורות: כתוב מסמך מפורט באורך מינימום 900–1400 מילים (או יותר לפי עומק החומר). כל סעיף ראשי חייב להיות עשיר בתוכן — לא כותרת עם משפט בודד.',
    'חובה לכלול את ארבעת הסעיפים עם כותרות ## ותתי־סעיפים ###/בולטים: (1) מבוא והמשגה פדגוגית; (2) תכנית העבודה ומפת הנושאים; (3) מתודולוגיה ודרכי הוראה; (4) מודלים של הערכה ומשימות סיכום — ואז מראי מקום.',
    'שלב בין מערכי שיעור, מחברות, דפי עבודה, מבחנים וחומרי רקע לכתיבה קוהרנטית אחת עם ## / ### ובולטים מפורטים.',
