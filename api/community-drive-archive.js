...
async function callGeminiModel(model, systemPrompt, userParts) {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      generationConfig: {
        temperature: 0.45,
        // High ceiling so long pedagogical summaries finish (avoid mid-sentence cutoffs).
...
function buildCommunitySummarySystemPrompt() {
  return [
    ...
    'תפקידך לנתח את כל הקבצים והחומרים המופיעים בתיקיית הנושא ולחלץ מתוכם תכנית עבודה לימודית מעמיקה, מקיפה ומורחבת עבור מורה המלמד את התקופה.',
    'אל תסתפק בתקציר גנרי! בצע סינתזה אמיתית בין כל קבצי התיקייה (מערכי שיעור, מחברות תלמידים, דפי עבודה, מבחנים, חומרי רקע) ובנה מסמך פדגוגי שלם.',
    ...
    'כתוב בעברית פדגוגית עשירה, רהוטה, מקצועית ועמוקה. סיים כל פסקה וכל סעיף בצורה מלאה — אל תקטע משפטים באמצע.',
    'דרישת אורך: כשיש תוכן במקורות, המסמך חייב להיות מפורט ומקיף — מינימום 600–900 מילים (רצוי יותר אם החומר מאפשר). אסור להסתפק בסיכומים שטחיים.',
    'מבנה Markdown חובה בתוך שדה summary — כותרת ראשית (#), ואז ארבעת הסעיפים הבאים בדיוק כ־## (עם תתי־סעיפים ב־### ובולטים):',
    '## 1. מבוא והמשגה פדגוגית — הרקע הפדגוגי וההתפתחותי של התקופה (התאמה לגיל/כיתה); השאלות המנחות והרעיון המרכזי העולה מהחומרים בתיקייה.',
    '## 2. תכנית העבודה ומפת הנושאים (מהחומרים בשטח) — סקירה מפורטת של הנושאים והפרקים הנלמדים (חלוקה לתתי־נושאים); שילוב תחומים ודגשים מיוחדים (אמנות, כתיבה, היסטוריה, מדעים וכו׳ כפי שמשתקף בקבצים).',
    '## 3. מתודולוגיה ודרכי הוראה — סגנונות השיעור והמתודות העולות מהמערכים והדפים; רעיונות לשיעורי ציור, כתיבה יוצרת, עבודה מעשית או עבודה בקבוצות.',
    '## 4. מודלים של הערכה ומשימות סיכום — בחינת חומרי ההערכה הקיימים בתיקייה (מבחנים, עבודות סיכום, דפי עבודה); מפרט דרישות לתלמיד: מבנה המבחן / קריטריונים לעבודת הסיכום (כולל מחוון/רובריקה במידה וקיים בקבצים). אם אין חומרי הערכה במקורות — ציין זאת במפורש ואל תמציא מבחן.',
    'הדגש מושגי מפתח ב־**מודגש**. בהדגשות Markdown שמור על זוגות תקינים של **…** באותה יחידת טקסט — אל תשבור ** באמצע שורה ללא סגירה.',
    'חובה להשלים את כל ארבעת הסעיפים עד הסוף — אל תעצור באמצע משפט, אל תקצר בגלל אורך, ואל תדלג על סעיפים.',
    ...
    'ודא שה-JSON שלם וסגור (סוגריים ומירכאות) ושדה summary מכיל את המסמך המלא מההתחלה ועד מראי המקום.',
  ].join(' ');
}
...
function buildCommunitySummaryTextPreamble(query, options, fileBundles, mediaMeta) {
  ...
  return [
    ...
    '',
    'נתח את כל חומרי תיקיית הנושא שלהלן ובנה מסמך תכנית עבודה לימודית מעמיקה ומורחבת למורה — סינתזה אמיתית בין המקורות, לא תקציר גנרי.',
    'כשיש חומר במקורות: כתוב מסמך מפורט באורך מינימום 600–900 מילים (או יותר לפי עומק החומר).',
    'חובה לכלול את ארבעת הסעיפים: (1) מבוא והמשגה פדגוגית; (2) תכנית העבודה ומפת הנושאים; (3) מתודולוגיה ודרכי הוראה; (4) מודלים של הערכה ומשימות סיכום — ואז מראי מקום.',
    'שלב בין מערכי שיעור, מחברות, דפי עבודה, מבחנים וחומרי רקע לכתיבה קוהרנטית אחת עם ## / ### ובולטים.',
    'כתוב עד להשלמה מלאה של כל הסעיפים כולל מראי מקום — בלי קיטוע באמצע משפט.',
