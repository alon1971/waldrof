...
(function (global) {
  ...
  function buildChatWordHtml(messages) {
    ...
    var html = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40">' +
      '<head><meta charset="utf-8"><title>' + deps.escapeHtml(title) + '</title>' +
      ...
      '<div class="meta-info">' + deps.escapeHtml(meta) + '</div>';

    messages.forEach(function (m) {
      if (m.role === 'user') {
        html += '<div class="section-user">' + userPrefix + deps.escapeHtml(m.text || '') + '</div>';
      } else {
        var body = m.html
          ? m.html
          : formatAssistantTextForWord(m.text || '').replace(/\n/g, '<br>');
        html += '<div class="section-assistant">' + body + '</div>';
      }
    });
