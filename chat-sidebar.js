...
(function (global) {
  ...
  var deps = {
    ...
    buildWordExportBlob: null,
    assertWordDownloadAllowed: null,
    recordWordDownload: null,
  ...
  function buildChatWordBodyHtml(messages) {
    var en = deps.isEnglish();
    var align = en ? 'left' : 'right';
    var title = en ? 'Activity summary — Pedagogy assistant' : 'סיכום פעילות ועבודה - עוזר פדגוגי';
    var meta = en
      ? 'Generated by the central digital system'
      : 'הופק באמצעות המערכת הדיגיטלית המרכזית';
    var userPrefix = en ? 'Teacher question / request: ' : 'שאלה / בקשת המורה: ';
    var html =
      '<div class="header-title" style="font-size: 22pt; font-weight: bold; margin-bottom: 5px; border-bottom: 2px solid #1e3a8a; padding-bottom: 10px; text-align: ' + align + ';">' +
        deps.escapeHtml(title) +
      '</div>' +
      '<div class="meta-info" style="color: #666666; margin-bottom: 30px; text-align: ' + align + ';">' +
        deps.escapeHtml(meta) +
      '</div>';

    messages.forEach(function (m) {
      if (m.role === 'user') {
        html += '<div class="section-user" style="font-weight: bold; color: #2563eb; margin-top: 20px; margin-bottom: 5px; background: #f0f4ff; padding: 8px; border-' + (en ? 'left' : 'right') + ': 3px solid #2563eb; text-align: ' + align + ';">' +
          userPrefix + deps.escapeHtml(m.text || '') + '</div>';
      } else {
        var body = m.html
          ? m.html
          : formatAssistantTextForWord(m.text || '').replace(/\n/g, '<br>');
        html += '<div class="section-assistant" style="color: #333333; margin-bottom: 25px; padding-' + (en ? 'left' : 'right') + ': 10px; text-align: ' + align + ';">' + body + '</div>';
      }
    });
  ...
  async function downloadChatMessagesDoc() {
    ...
    if (typeof deps.assertWordDownloadAllowed === 'function') {
      try {
    ...
    var panelBtn = document.getElementById('lesson-chat-export-word');
    var fsBtn = document.getElementById('lesson-chat-fullscreen-export-word');
    var panelLabel = panelBtn ? panelBtn.querySelector('.lesson-chat-export-label') : null;
    var fsLabel = fsBtn ? fsBtn.querySelector('.lesson-chat-export-label') : null;
    ...
    try {
      ...
      // Prefer shared download helper (anchor + .docx MIME) so Save As is not HTML.
      ...
      alert((deps.t('export_doc_error') || 'שגיאה בהפקת המסמך') + ': ' + (err.message || String(err)));
    } finally {
      if (panelBtn) panelBtn.disabled = false;
      if (fsBtn) fsBtn.disabled = false;
      syncChatExportLabel(panelBtn);
      syncChatExportLabel(fsBtn);
    }
  }
  ...
  function init(options) {
    ...
    if (typeof options.buildWordExportBlob === 'function') deps.buildWordExportBlob = options.buildWordExportBlob;
    if (typeof options.assertWordDownloadAllowed === 'function') deps.assertWordDownloadAllowed = options.assertWordDownloadAllowed;
    if (typeof options.recordWordDownload === 'function') deps.recordWordDownload = options.recordWordDownload;
