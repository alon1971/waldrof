/**
 * Google Drive Γזע community_materials catalog sync with recursive subfolder traversal.
...
/** Larger ceiling for multimodal Gemini PDF/image fallback (Gemini PDF cap Γיט 50MB). */
...
/** In-memory Drive folder tree cache (grade/topic Γזע folder ids). */
...
const HEBREW_GRADE_TO_ID = {
  '╫נ': '1', '╫ס': '2', '╫ע': '3', '╫ף': '4', '╫פ': '5', '╫ץ': '6', '╫צ': '7', '╫ק': '8',
};
  ...
  try {
    ...
    if (!parsed || typeof parsed !== 'object' || !parsed.client_email || !parsed.private_key) {
      console.warn(
        '[drive-catalog-sync]',
        label,
        'is set but missing client_email/private_key Γאפ Drive disabled'
      );
      return null;
    }
    return parsed;
...
/**
 * User OAuth (installed / desktop or web client) Γאפ files are owned by the human
...
async function saveDriveOauthCredentials(options) {
  ...
  if (!res.ok) {
    const hint = /relation|does not exist|PGRST205/i.test(text)
      ? ' Γאפ run supabase/drive_oauth_credentials.sql'
      : '';
    throw new Error(
...
async function exchangeOauthRefreshToken(creds) {
  ...
  if (!res.ok) {
    const err = new Error(
      ...
      + ' Γאפ re-run: node scripts/google-drive-oauth-setup.js'
    );
...
async function resolveDriveAccessToken(options) {
  ...
  // 1) User OAuth Γאפ preferred for writes (uses the owner's storage quota).
  ...
  // 2) SA with domain-wide delegation (Workspace) Γאפ acts as the delegate user.
  const delegate = getDriveDelegateEmail();
  if (wantWrite && delegate) {
    return exchangeServiceAccountJwt(sa, scope, delegate);
  }

  // 3) Plain SA Γאפ works for reads + Shared Drive writes; My Drive uploads fail.
...
function parseGradeIdFromFolderName(name) {
  ...
  if (/^(╫¢╫£╫£╫ש|general)$/iu.test(s) || /^╫¬╫ש╫º╫ש[╫ש╫פ]\s+╫¢╫£╫£╫ש/iu.test(s)) {
    return 'general';
  }
  ...
  // ┬½╫¢╫ש╫¬╫פ ╫צ╫│┬╗, ┬½╫¢╫ש╫¬╫פ-╫צ┬╗, ┬½╫¢╫ש╫¬╫פ ╫צ ╫¬╫⌐╫ñ╫ץ┬╗, ┬½╫ס╫¢╫ש╫¬╫פ ╫צ'┬╗
  const heb = s.match(/(?:^|[\s\-_/])(?:╫ץ|╫ס|╫£|╫⌐)?╫¢╫ש╫¬╫פ[\s\-_/]*([╫נ-╫ק])['╫│"]?/u)
    || s.match(/^([╫נ-╫ק])['╫│"]?\s*$/u);
  if (heb) return HEBREW_GRADE_TO_ID[heb[1].charAt(0)] || '';

  if (/^╫¢╫ש╫¬╫פ[\s\-_/]*[╫נ-╫ק]['╫│"]?$/iu.test(s)) {
    return HEBREW_GRADE_TO_ID[s.replace(/^╫¢╫ש╫¬╫פ[\s\-_/]*/iu, '').charAt(0)] || '';
  }
...
/** Human folder path for citations, e.g. "╫¢╫ש╫¬╫פ ╫פ╫│ > ╫ש╫ץ╫ץ╫ƒ". */
...
async function resolveShortcutTarget(shortcutFile, accessToken) {
  ...
  if (!targetId) {
    // List responses sometimes omit nested shortcutDetails Γאפ re-fetch the shortcut shell.
    try {
  ...
  try {
    ...
    if ((isDriveNotFoundError(err) || (err && err.statusCode === 404)) && targetMimeType) {
      console.warn(
        '[drive-catalog-sync] shortcut target meta 404 Γאפ soft-resolving for extract:',
        shortcutFile.name || shortcutFile.id,
        'Γזע',
        targetId,
        targetMimeType,
...
async function walkDriveFolderTree(folderId, ctx, accessToken, stats) {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (item.mimeType === FOLDER_MIME) {
      ...
      if (!ctx.gradeId) {
        ...
        if (gradeId) {
          ...
          console.log('[drive-catalog-sync] matched grade folder:', childName, 'Γזע grade', gradeId);
        }
      } else if (!ctx.catalogTopic) {
        const topic = catalogTopics.resolveCatalogTopicFromFolderName(childName);
        nextCtx.catalogTopic = topic;
        console.log('[drive-catalog-sync] inherited catalog topic from folder:', childName, 'Γזע', topic);
      }
    ...
    const catalogTopic = inheritedTopic || '╫¢╫£╫£╫ש';
    ...
    try {
      const result = await upsertDriveCatalogMaterial({
        ...
        topic: catalogTopic,
        fileUrl: fileUrl,
        fileName: item.name || '╫º╫ץ╫ס╫Ñ Drive',
        notes: notes,
        driveFileId: item.id,
...
async function syncCommunityDriveCatalog(options) {
  ...
  console.log(
    '[drive-catalog-sync] complete Γאפ files:', stats.filesScanned,
    'added:', stats.added,
...
function backgroundFetchDriveCatalogAsync(options) {
  if (!isDriveCatalogSyncConfigured() && !(options && options.accessToken)) {
    console.warn(
      '[drive-catalog-sync] background fetch skipped Γאפ Drive env not configured (see docs/google-drive-setup.md)'
    );
    return;
  }
...
/** Boot / ops helper Γאפ never throws; logs why Drive is unavailable. */
function logDriveConfigStatus(prefix) {
  ...
  if (!sa && !oauth) {
    console.warn(
      tag,
      'not configured Γאפ set GOOGLE_DRIVE_SERVICE_ACCOUNT_JSON and/or GOOGLE_DRIVE_OAUTH_* '
        + '(see docs/google-drive-setup.md). Community search will use local/index fallback only.'
    );
  } else {
    console.warn(tag, 'incomplete Γאפ catalog root folder id is missing');
  }
  return false;
}
 ...
 * Example: (fullText contains '╫¿╫ץ╫₧╫נ' or name contains '╫¿╫ץ╫₧╫נ') and trashed=false
 */
 ...
 * Used only by the Community Archive tab overview Γאפ not by Gemini summarizer.
 */
...
/**
 * Live Drive overview for ╫₧╫נ╫ע╫¿ ╫º╫פ╫ש╫£╫¬╫ש grade cards:
...
function topicsStrictlyCompatible(expectedTopic, candidateTopic) {
  ...
  // Fuzzy includes (hyphens/spaces already normalized): ┬½╫¬╫צ╫ץ╫á╫פ┬╗ Γזפ ┬½╫¬╫צ╫ץ╫á╫פ ╫ץ╫ס╫¿╫ש╫נ╫ץ╫¬┬╗.
  ...
  // Active semantic exclude (e.g. Norse Γזפ Greek/Roman mythology).
  if (
  ...
  // Alias substring: folder ┬½╫¬╫צ╫ץ╫á╫פ ╫ץ╫₧╫ó╫¿╫¢╫ש ╫⌐╫ש╫ó╫ץ╫¿┬╗ vs alias ┬½╫¬╫צ╫ץ╫á╫פ┬╗.
...
function nameMatchesTopicCentrally(fileName, topic, query) {
  ...
  for (const seed of seeds) {
    ...
    // Construct / morphological near-match: ┬½╫¬╫צ╫ץ╫á╫¬Γאª┬╗ Γזפ ┬½╫¬╫צ╫ץ╫á╫פ┬╗
  ...
  for (let i = 0; i < parents.length; i++) {
    ...
    if (meta && meta.gradeId) {
      return {
        ...
        catalogTopic: meta.catalogTopic || '╫¢╫£╫£╫ש',
...
function formatDriveSearchHit(file, scope, query) {
  const fileName = String((file && file.name) || '').trim() || '╫º╫ץ╫ס╫Ñ Drive';
  const qNorm = stableNormalize(query);
  const nameNorm = stableNormalize(fileName);
  const nameHit = Boolean(qNorm && nameNorm && nameNorm.indexOf(qNorm) >= 0);
  const catalogTopic = scope.catalogTopic || '╫¢╫£╫£╫ש';
  ...
  return {
    ...
    alertText: catalogTopic
      ? ('╫á╫₧╫ª╫נ ╫ק╫ץ╫₧╫¿ ╫ס╫¬╫ש╫º╫ש╫ש╫¬ ┬½' + catalogTopic + '┬╗ ╫ס-Google Drive')
      : '',
  };
}
...
function resolveDriveSearchScope(query, options) {
  ...
  // a grade lock from curriculum inference Γאפ only an explicit UI/query grade may.
  ...
  // UI / query grade lock wins Γאפ never expand Drive search into another classroom.
  ...
  if (!strictTopic) {
    ...
    if (fromFolder) {
      ...
      // Lock to the alias that actually matched the query (╫ע╫ש╫נ╫ץ╫£╫ץ╫ע╫ש╫פ Γיá ╫¿╫ץ╫₧╫נ).
      strictTopic = pickBestMatchingAlias(q, block.aliases);
    }
    if (!strictTopic) {
      if (qNorm === '╫¿╫ץ╫₧╫נ' || qNorm.indexOf('╫¿╫ץ╫₧╫נ') === 0) strictTopic = '╫¿╫ץ╫₧╫נ';
      else if (qNorm === '╫ש╫ץ╫ץ╫ƒ' || qNorm.indexOf('╫ש╫ץ╫ץ╫ƒ') === 0) strictTopic = '╫ש╫ץ╫ץ╫ƒ';
      else if (qNorm.indexOf('╫á╫ץ╫¿╫ף') >= 0 || qNorm.indexOf('norse') >= 0) {
        strictTopic = '╫₧╫ש╫¬╫ץ╫£╫ץ╫ע╫ש╫פ ╫á╫ץ╫¿╫ף╫ש╫¬';
      }
    }
  }
 ...
 * Never filters by signed-in userId Γאפ the catalog is global/community scope only.
 */
async function searchDriveCommunityCatalog(query, options) {
  ...
  if (branchFolderIds.size) {
    ...
    if (!effectiveAllowed.size) {
      // Topic lock under this parent found nothing Γאפ keep the whole grade branch.
  ...
  // Prefer multi-word pedagogical phrases; avoid bare tokens like "╫נ╫ף╫¥" that pollute fullText.
  ...
  // search the catalog root / ungraded layers by bare topic name Γאפ materials like
  // ┬½╫¬╫צ╫ץ╫á╫פ ╫ץ╫á╫⌐╫ש╫₧╫פΓאª┬╗ often live as root shortcuts outside ╫¢╫ש╫¬╫פ folders.
  ...
  rawFiles.forEach(function (file) {
    ...
    if ((!fileScope || !fileScope.gradeId) && file.mimeType === FOLDER_MIME && index.byFolderId[file.id]) {
      ...
      fileScope = {
        ...
        gradeId: meta.gradeId || '',
        catalogTopic: meta.catalogTopic || file.name || '╫¢╫£╫£╫ש',
        path: (meta.path || []).slice(),
      };
    }
    ...
    if (!inScope) {
      ...
      fileScope = {
        ...
        path: (fileScope && fileScope.path && fileScope.path.length)
          ...
          : (scope.gradeId
            ? [(pedagogicalScope.GRADE_LABEL_BY_ID[scope.gradeId] || ('╫¢╫ש╫¬╫פ ' + scope.gradeId)), fileScope && fileScope.catalogTopic].filter(Boolean)
            : ['╫₧╫נ╫ע╫¿ ╫º╫פ╫ש╫£╫¬╫ש']),
    ...
    if (scope.topic) {
      ...
      if (!topicOk) {
        ...
        // keyword is central in the file name Γאפ never sibling-folder fullText hits.
    ...
    // Reject fullText-only false positives (e.g. ╫ע╫ש╫£╫ע╫₧╫⌐ for ┬½╫נ╫ף╫¥ ╫ק╫ש╫פ┬╗).
...
function uniquePrioritySearchTerms(query, topic, existingTerms) {
  ...
  add(query);
  add(topic);
  // Morphological Hebrew stems so Drive `name contains` finds ┬½╫¬╫צ╫ץ╫á╫¬Γאª┬╗ for ┬½╫¬╫צ╫ץ╫á╫פ┬╗.
...
async function listDriveFilesForGradeTopic(gradeId, topic, options) {
  ...
  function tryAdmitListedFile(item, folderId, asUngradedFallback) {
    ...
    if (!fileScope || !fileScope.gradeId) {
      ...
      fileScope = {
        ...
        path: (folderMeta && folderMeta.path)
          ...
          : (asUngradedFallback ? ['╫₧╫נ╫ע╫¿ ╫º╫פ╫ש╫£╫¬╫ש'] : []),
    ...
    if (topicStr) {
      ...
      // entire grade tree into the summarizer Γאפ require central topic evidence.
      if (!topicOk) {
        debugInfo.rejected.push({
          name: item.name,
  ...
  console.log(
    '[drive-catalog-sync] listing topic-folder files Γאפ grade:',
    gid,
    '| topic:',
    topicStr || '(any)',
    '| folders:',
...
async function probeDriveCachedFileMetadata(fileRefsOrIds, options) {
  ...
  console.log(
    '[drive-catalog-sync] focused cache probe Γאפ files.get for',
    refs.length,
