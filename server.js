...
async function handleApiArchiveLink(req, res) {
  const apiRes = createApiResponse(res);
  try {
    let body;
    if (req.method === 'POST' || req.method === 'PUT' || req.method === 'PATCH') {
    ...
    await archiveLinkApi.legacyHandler({
      method: req.method,
      headers: req.headers,
      body: body,
      url: req.url,
...
async function handleApiTts(req, res) {
  const apiRes = createApiResponse(res);
  try {
    let body;
    if (req.method === 'POST' || req.method === 'PUT' || req.method === 'PATCH') {
    ...
    await ttsApi.legacyHandler({ method: req.method, headers: req.headers, body: body }, apiRes);
  } catch (err) {
    if (!res.headersSent) {
      apiRes.status(500).json({
        ok: false,
...
const server = http.createServer(async function (req, res) {
  ...
  if (pathname === '/api/archive-link' || pathname === '/api/delete-link') {
    return handleApiArchiveLink(req, res);
  }
